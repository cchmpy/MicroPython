# 使用Signal类修改的7-4.py程序，可简化状态获取
import time
from machine import Pin, Signal
class Button:
    DOWN, UP = 1, 0    # 按键状态：按下、松开 <---change--->
    E_DOWN   = 1       # 事件：按下
    E_UP     = 2       # 事件：短按松开
    E_LONG_UP= 4       # 事件：长按松开
    
    def __init__(self, pin, pull_up=True, debounce=50, long_press=1000): 
        assert isinstance(pin,Pin),'pin must be an instance of Pin'  # 断言: 判定pin是否是Pin的实例
        self.pin       = pin              # 引脚对象，pin可定义使用内部还是外部上拉/下拉电阻
        self.signal    = Signal(pin, invert = pull_up)               # <---change--->    
        ...
        self.status    = self.signal()                      # 获取按键初始状态<---change---> 
    ...
    def _handler(self, pin):
        ...
        if  interval> self.debounce:                        # 忽略抖动，仅处理有效按键动作
            s = self.signal()                               # 中断触发时按键状态<---change--->
